...
function getScenarioOptions() {
  ...
  if (currentScenario?.hasCustomParams && refs.customParamsContainer) {
    const customParams = {};
    for (const p of currentScenario.params) {
      const input = refs.customParamsContainer.querySelector(`[data-param="${p.key}"]`);
      customParams[p.key] = input ? Number(input.value) : p.default;
    }
  ...
  container.innerHTML = scenario.params
    .map(
      ...
      <label class="bench-param">
        ...
        <span class="bench-param__value" data-param-display="${p.key}">${p.default}</span>
      </label>`
    )
    .join("");
