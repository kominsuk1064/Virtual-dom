...
/**
 * Escape text before writing it into innerHTML.
 */
...
/**
 * Serialize a VDOM tree into HTML text in the parent document.
 */
...
function parseHtmlInput(html) {
  ...
  if (hasMeaningfulText) {
    return {
      valid: false,
      message: "맨 바깥에는 공백이 아닌 텍스트를 둘 수 없습니다.",
      vdom: null
    };
  }
  ...
  if (elementNodes.length !== 1 || topLevelNodes.length !== 1) {
    return {
      valid: false,
      message: "맨 바깥 요소는 정확히 1개만 허용됩니다.",
      vdom: null
    };
  }
  ...
  if (!nextVdom) {
    return {
      valid: false,
      message: "입력한 HTML을 정규화된 가상 DOM으로 바꾸지 못했습니다.",
      vdom: null
    };
  }
  ...
  return {
    valid: true,
    message: `루트 <${nextVdom.type}>를 비교용 후보 화면으로 준비했습니다.`,
    vdom: nextVdom
  };
}
...
function renderChangeSummary(mode, options = {}) {
  ...
  if (mode === "synced") {
    ...
    renderSummaryEntries([
      ...
      {
        title: "다음 행동",
        text: "입력창을 수정하거나 프리셋 시나리오를 눌러 새로운 변화를 만들어 보세요."
      }
    ]);
    return;
  }
  ...
  renderSummaryEntries([
    {
      ...
      text: "선택한 이력 상태를 기준으로 실제 화면, 후보 화면, 입력창을 한 번에 다시 맞췄습니다."
    },
    {
      title: "왜 전체 렌더를 하나요?",
...
/**
 * Build a presenter-friendly draft HTML from the current actual DOM.
 */
function buildScenarioDraft(kind) {
  ...
  if (kind === "sync") {
    return {
      html: vdomToHtmlString(currentVDOM),
...
/**
 * Apply a preset scenario to the editor.
 */
function applyScenario(kind) {
  ...
  const validation = refreshPreviewFromTextarea(`${scenarioLabels[kind]} 시나리오를 적용했습니다.`);
 ...
 */
function refreshPreviewFromTextarea(reason = "입력창 기준으로 후보 화면을 다시 계산했습니다.") {
...
function syncBothAreasFromVdom(vdom) {
  ...
  htmlInput.value = vdomToHtmlString(snapshot);

  currentValidation = {
    valid: true,
    message: "실제 페이지와 후보 페이지, 입력창을 현재 이력 상태로 다시 맞췄습니다.",
    vdom: snapshot
  };
  pendingPatches = [];
...
function handlePatch() {
  const validation = refreshPreviewFromTextarea("반영 직전 비교를 다시 계산했습니다.");
  ...
  if (!patches.length) {
    ...
    renderPatchLog("변화 없음", [], [
      "[안내] 현재 입력창의 정규화된 가상 DOM이 실제 화면 상태와 동일합니다."
    ]);
...
function init() {
  ...
  renderPatchLog("준비 완료", [], [
    ...
    "[다음 단계] 시나리오 버튼이나 입력창을 사용해 오른쪽 후보 화면을 먼저 바꿔 보세요."
  ]);
  scenarioMessage.textContent = "프리셋 시나리오 또는 입력창 수정으로 다음 후보 화면을 만들어 보세요.";
...
async function bootstrap() {
  ...
  htmlInput.addEventListener("input", () => {
    refreshPreviewFromTextarea("입력창 수정으로 후보 화면을 다시 계산했습니다.");
  });
